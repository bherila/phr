/**
 * Classic marching cubes lookup tables (edge table + triangulation table),
 * derived from the public-domain algorithm originally published by
 * Lorensen & Cline (1987) and popularized by Paul Bourke's
 * "Polygonising a scalar field" reference implementation.
 *
 * Cube corner encoding: corner index `i` in [0, 7] maps to local unit-cube
 * offsets `(x, y, z) = (i & 1, (i & 2) >> 1, (i & 4) >> 2)`. A case index is
 * built by setting bit `i` when corner `i`'s field value is "inside" the
 * surface (see marchingCubes.ts for the inside/outside convention).
 *
 * Cube edge encoding (0-11), as (cornerA, cornerB) pairs of the corner
 * indices above:
 *   0: (0,1)  1: (1,3)  2: (3,2)  3: (2,0)
 *   4: (4,5)  5: (5,7)  6: (7,6)  7: (6,4)
 *   8: (0,4)  9: (1,5) 10: (3,7) 11: (2,6)
 *
 * This is the closed-mesh-safe variant of the classic tables (edges/corners
 * are numbered so every cell case reduces to axis-aligned edges with an
 * unambiguous "lower corner", which also makes cross-cell vertex dedup
 * straightforward). It is geometrically equivalent to the original
 * Lorensen/Bourke tables, just with a corner/edge numbering chosen for
 * implementation simplicity and to avoid the ambiguous-face cracks some
 * naive transcriptions of the original table are prone to.
 *
 * EDGE_TABLE[caseIndex] is a 12-bit mask; bit e is set when edge e is
 * crossed by the isosurface for that case.
 *
 * TRI_TABLE[caseIndex] lists the crossed-edge indices for each triangle of
 * that case, flattened and grouped in triples (edge0, edge1, edge2, edge0,
 * edge1, edge2, ...). Rows are stored WITHOUT the classic -1 sentinel
 * padding; a case with no triangles is an empty array.
 */
