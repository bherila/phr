    ...
    public function definitions(AgentMcpReadTools $reads, AgentMcpWriteTools $writes): array
    {
        $definitions = [
            ...
            new ToolDefinition(
                'patients.create',
                'Create patient',
                'Create a new PHR patient owned by the authorized account and grant that account owner-level access to it.',
            ...
            $this->method('dicom_studies.list', 'List imaging studies', 'List bounded DICOM study metadata without returning image pixels or instance files.', $reads, 'dicomStudiesList'),
            $this->method('dicom_studies.get', 'Get imaging study', 'Get one DICOM study metadata record without returning image pixels or instance files.', $reads, 'dicomStudiesGet'),
            $this->method('dicom_series.list', 'List imaging series', 'List bounded series metadata for one DICOM study without returning image pixels or instance metadata.', $reads, 'dicomSeriesList'),
            new ToolDefinition(
                'dicom_uploads.open',
                'Open DICOM upload',
