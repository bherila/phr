...
class PhrPatientSearchController extends Controller
{
    ...
    private function officeVisits(int $patientId, string $query, int $limit): Collection
    {
        ...
        ])->latest('visit_date')->limit($limit)->get()->map(fn (PhrOfficeVisit $visit): array => $this->result(
    ...
    private function imagingStudies(int $patientId, string $query, int $limit): Collection
    {
        ...
        ])->latest('study_date')->limit($limit)->get()->map(fn (PhrDicomStudy $study): array => $this->result(
    ...
    private function documents(int $patientId, string $query, int $limit): Collection
    {
        ...
        ])->latest('observed_at')->limit($limit)->get()->map(fn (PhrDocument $document): array => $this->result(
    ...
    private function procedures(int $patientId, string $query, int $limit): Collection
    {
        ...
        ])->latest('performed_on')->limit($limit)->get()->map(fn (PhrProcedure $procedure): array => $this->result(
    ...
    private function immunizations(int $patientId, string $query, int $limit): Collection
    {
        ...
        ])->latest('administered_on')->limit($limit)->get()->map(fn (PhrImmunization $immunization): array => $this->result(
    ...
    private function medications(int $patientId, string $query, int $limit): Collection
    {
        ...
        ])->latest('started_on')->limit($limit)->get()->map(fn (PhrMedication $medication): array => $this->result(
    ...
    private function conditions(int $patientId, string $query, int $limit): Collection
    {
        ...
        ])->latest('onset_date')->limit($limit)->get()->map(fn (PhrCondition $condition): array => $this->result(
    ...
    private function labs(int $patientId, string $query, int $limit): Collection
    {
        ...
        ])->latest('result_datetime')->limit($limit)->get()->map(fn (PhrLabResult $lab): array => $this->result(
    ...
    /** @param Builder<Model> $records @return Builder<Model> */
